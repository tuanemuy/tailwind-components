...
export const FilesListView = ({ items = defaultItems }: FilesListViewProps) => {
  return (
    <div>
      {items.map((item) => (
        <div
          key={item.id}
          className="relative group bg-card border border-border -mt-px first:mt-0 first:rounded-t-xl last:rounded-b-xl"
        >
          <a
            className="group p-3 flex items-center gap-x-4 group-first:rounded-t-xl group-last:rounded-b-xl hover:bg-muted focus:outline-hidden focus:bg-muted"
            href={item.href || "#"}
          >
