...
export interface PrimaryColorStatsWithBrandsAndBadgesProps {
  cards?: StatCard[];
...
export const PrimaryColorStatsWithBrandsAndBadges = ({
  cards = defaultCards,
  trendUpIcon,
  trendDownIcon,
}: PrimaryColorStatsWithBrandsAndBadgesProps) => {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 border border-border shadow-2xs rounded-xl overflow-hidden">
      {cards.map((card, index) => {
        const isFirst = index === 0;
        const borderClass = isFirst
          ? ""
          : index === 1
            ? "border-s border-t sm:border-t-0"
            : index === 2
              ? "sm:border-s-0 md:border-s border-t sm:border-t md:border-t-0"
              : "border-s border-t sm:border-t md:border-t-0";

        return (
          <div
            key={index}
            className={`flex flex-col p-5 bg-card ${borderClass} ${!isFirst ? "border-border" : ""}`}
          >
            {card.icon}
            <div className="mt-3">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-y-1 gap-x-2">
                ...
                <span className="py-1 px-1.5 inline-flex items-center gap-x-1 text-xs bg-secondary text-foreground rounded-md">
                  {renderTrendIcon(card.trendDirection)}
                  {card.changePercent}
                </span>
              </div>
            </div>
