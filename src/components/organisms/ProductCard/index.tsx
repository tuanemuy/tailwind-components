...
import { Button, Badge, type BadgeProps } from "@/components/atoms";
import { IconButton } from "@/components/molecules";
import { HeartIcon, StarIcon, ShoppingBagIcon, EyeIcon } from "@/lib/icons";
...
// Badge configuration
const badgeConfig: Record<ProductBadgeType, { variant: BadgeProps["variant"]; defaultLabel: string }> = {
...
export const ProductCardImage = forwardRef<HTMLDivElement, ProductCardImageProps>(
  (
    {
      className,
      src,
    ...
    return (
      <div
        ref={ref}
        className={cn("relative overflow-hidden bg-muted", aspectRatioClasses[aspectRatio], className)}
        {...props}
      >
        <img
          src={src}
        ...
        {badges.length > 0 && (
          <div className="absolute top-3 left-3 flex flex-col gap-y-1">
            {badges.map((badge, index) => {
              ...
              return (
                <Badge key={index} variant={config.variant} size="sm">
        ...
        {showQuickActions && (
          <div className="absolute top-3 right-3 flex flex-col gap-y-2 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
            {onFavoriteClick && (
              <IconButton
                icon={<HeartIcon className={cn("size-4", isFavorite && "fill-current text-destructive")} />}
                size="sm"
                variant="secondary"
...
export const ProductCardBody = forwardRef<HTMLDivElement, ProductCardBodyProps>(
  ({ className, padding = "md", children, ...props }, ref) => {
    return (
      <div ref={ref} className={cn("flex flex-col gap-y-2", paddingClasses[padding], className)} {...props}>
        {children}
      </div>
    );
  },
...
export const ProductCardTitle = forwardRef<HTMLHeadingElement, ProductCardTitleProps>(
...
export interface ProductCardCategoryProps extends React.HTMLAttributes<HTMLSpanElement> {}

export const ProductCardCategory = forwardRef<HTMLSpanElement, ProductCardCategoryProps>(
  ({ className, children, ...props }, ref) => {
    return (
      <span ref={ref} className={cn("text-xs text-muted-foreground uppercase tracking-wide", className)} {...props}>
        {children}
      </span>
    );
  },
...
export const ProductCardRating = forwardRef<HTMLDivElement, ProductCardRatingProps>(
      ...
      <div ref={ref} className={cn("flex items-center gap-x-1", className)} {...props}>
        <div className="flex items-center">
          ...
          {Array.from({ length: fullStars }).map((_, i) => (
            <StarIcon key={`full-${i}`} className="size-4 fill-warning text-warning" />
          ))}
          {/* Half star */}
          {hasHalfStar && (
          ...
          {Array.from({ length: emptyStars }).map((_, i) => (
            <StarIcon key={`empty-${i}`} className="size-4 text-muted-foreground/30" />
          ))}
        </div>
...
export const ProductCardPrice = forwardRef<HTMLDivElement, ProductCardPriceProps>(
  ({ className, price, ...props }, ref) => {
    ...
    return (
      <div ref={ref} className={cn("flex items-center gap-x-2", className)} {...props}>
        <span className={cn("text-lg font-semibold", hasDiscount ? "text-destructive" : "text-foreground")}>
...
export const ProductCardActions = forwardRef<HTMLDivElement, ProductCardActionsProps>(
  ({ className, align = "stretch", children, ...props }, ref) => {
    return (
      <div ref={ref} className={cn("flex items-center gap-x-2 pt-2", alignClasses[align], className)} {...props}>
        {children}
      </div>
    );
  },
...
export interface ProductAddToCartButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: "default" | "icon";
  loading?: boolean;
}

export const ProductAddToCartButton = forwardRef<HTMLButtonElement, ProductAddToCartButtonProps>(
  ({ className, variant = "default", loading = false, children, ...props }, ref) => {
    ...
    return (
      <Button ref={ref} variant="primary" disabled={loading} className={cn("w-full", className)} {...props}>
        {loading ? (
          "Adding..."
        ) : (
          <>
