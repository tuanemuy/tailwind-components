...
import { FolderIcon, MoreHorizontalIcon, StarIcon, LockIcon, UsersIcon } from "@/lib/icons";
import { folderColorConfig, type FolderColor } from "@/lib/variants";
...
export interface FolderCardProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "onSelect"> {
  folder: FolderData;
  ...
  (
    ...
    if (variant === "list") {
      return (
        ...
        >
          <div className="flex items-center gap-x-3">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-x-2">
                ...
                {folder.isStarred && <StarIcon className="size-3.5 fill-warning text-warning" />}
                {folder.isPrivate && <LockIcon className="size-3.5 text-muted-foreground" />}
                {folder.isShared && <UsersIcon className="size-3.5 text-muted-foreground" />}
              </div>
              <div className="flex items-center gap-x-2 text-xs text-muted-foreground">
                {folder.itemCount !== undefined && <span>{folder.itemCount} items</span>}
          ...
          <div className="flex items-center gap-x-2">
            {folder.modifiedDate && (
              <span className="text-xs text-muted-foreground">{folder.modifiedDate}</span>
            )}
            <Button
              variant="ghost"
              size="sm"
      ...
      return (
        ...
        >
          <div className="relative">
            ...
            <div className={cn("flex size-16 items-center justify-center rounded-xl", colorClass)}>
          ...
          <h4 className="mt-3 line-clamp-2 text-sm font-medium text-foreground">{folder.name}</h4>
    ...
    return (
      ...
      >
        ...
        <div className="mt-3">
          <div className="flex items-center gap-x-2">
            <h4 className="truncate font-medium text-foreground">{folder.name}</h4>
            {folder.isPrivate && <LockIcon className="size-3.5 text-muted-foreground" />}
            {folder.isShared && <UsersIcon className="size-3.5 text-muted-foreground" />}
          </div>
          {folder.description && (
...
export interface FolderCardGridProps extends React.HTMLAttributes<HTMLDivElement> {
  folders: FolderData[];
...
export interface FolderCardListProps extends React.HTMLAttributes<HTMLDivElement> {
  folders: FolderData[];
