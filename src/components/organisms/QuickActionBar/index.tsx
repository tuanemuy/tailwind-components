        ...
        {actions.map((action, index) => (
          <button
            ...
            className={cn(
              ...
              action.variant !== "primary" && action.variant !== "destructive" && "hover:bg-muted"
            )}
          >
            <span className={cn(
...
export interface SelectionActionBarProps extends Omit<QuickActionBarProps, "actions"> {
...
export const SelectionActionBar = forwardRef<HTMLDivElement, SelectionActionBarProps>(
  (
    {
      className,
      position = "bottom",
    ...
    return (
      <div
        ref={ref}
        className={cn(quickActionBarVariants({ position, size }), "gap-x-4", className)}
        {...props}
      >
        {/* Selection info */}
...
export interface ContextActionBarProps extends React.HTMLAttributes<HTMLDivElement> {
  actions: Array<{
    icon: React.ReactNode;
    label: string;
    onClick: () => void;
...
export const ContextActionBar = forwardRef<HTMLDivElement, ContextActionBarProps>(
