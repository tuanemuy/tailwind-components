    ...
    return (
      ...
      >
        {items
          ? items.map((item) => (
              <DescriptionListItem
                key={`${String(item.term)}-${String(item.details)}`}
                variant={variant as DescriptionListVariant}
                size={size}
              >
                {item.icon && (
                  <DescriptionListIcon size={size}>
                    {item.icon}
                  </DescriptionListIcon>
                )}
                <DescriptionTerm
...
export interface DescriptionListItemProps
  ...
  term?: React.ReactNode;
  /** Icon to display before the term */
  icon?: React.ReactNode;
}
...
>(({ className, variant = "default", term, icon, children, ...props }, ref) => {
  return (
    <div
      ref={ref}
...
export const DescriptionListIcon = forwardRef<
  HTMLDivElement,
  DescriptionListIconProps
...
export const HorizontalDescriptionList = forwardRef<
  HTMLDListElement,
...
export const CardDescriptionList = forwardRef<
  HTMLDListElement,
        ...
        {items?.map((item) => (
          ...
          >
            <div className="flex items-center gap-x-3">
              ...
              <dt
                className={cn(descriptionTermVariants({ size }), "sm:w-auto")}
              >
                {item.term}
              </dt>
            </div>
